...
void GuassianModel::trainModel() {
	if (guassianNum == 1)
		trainSingleGuassian();
	else
		trainMixtureGuassian();
}
...
void GuassianModel::trainSingleGuassian() {
	for (int i = 0; i < guassianNum; i++) {
		for (int j = 0; j < FEATURE_LENGTH; j++) {
			means[i][j] = 0;
			variances[i][j] = 0;
...
void GuassianModel::trainMixtureGuassian() {
	for (int i = 0; i < guassianNum; i++) {
		for (int j = 0; j < FEATURE_LENGTH; j++) {
...
void GuassianModel::getPosterior(double **posterior, vector<double*> points, int pointNum) {
	for (int i = 0; i < pointNum; i++) {
...
void GuassianModel::updateParameters(double **posterior, vector<double*> points, int pointNum) {
...
double GuassianModel::getNegativeLogLikelihood(vector<double*> points, int pointNum) {
	double result = 0;
	for (int i = 0; i < pointNum; i++) {
