...
PENALTY = 200
MODEL_DIR = "./model_continuous_gmm4/"
GAUSSIAN_NUM = 4
...
class Node:
    def __init__(self, set_description="", set_is_non_emmit=False):
        ...
        self.mean_s = []      # 39 dimension mfcc sequence
        self.var_s = []
        self.weight_s = []
...
def temp_iso_model_gen(name):
    ...
    mean_s = []
    var_s = []
    for gaussian_cnt in range(GAUSSIAN_NUM):
        mean_s.append(np.loadtxt(MODEL_DIR + name[digit_index] + "gmm_mean"+str(gaussian_cnt+1)+".txt"))
        var_s.append(np.loadtxt(MODEL_DIR+name[digit_index]+"gmm_var"+str(gaussian_cnt+1)+".txt"))
    weight_s = np.loadtxt(MODEL_DIR + name[digit_index] + "gmm_weight.txt")
    stay_trans_cost = np.loadtxt(MODEL_DIR + name[digit_index] + "gmm_self_trans.txt")
    leave_trans_cost = np.loadtxt(MODEL_DIR + name[digit_index] + "gmm_trans_p.txt")

    for i in range(5):
        node = Node(name+"_state_"+str(i))
        for gaussian_cnt in range(GAUSSIAN_NUM):
            node.mean_s.append(mean_s[gaussian_cnt][i])
            node.var_s.append(var_s[gaussian_cnt][i])
        node.weight_s = weight_s[i]
...
def distance_gmm(vector, mean_s, var_s, weight_s):
    """"Cal the Gaussian distance between input vector and the jth mean vector"""
    probability_total = 0
    for gaussian_index in range(GAUSSIAN_NUM):
        if weight_s[gaussian_index] == 0:
            continue
        part1 = weight_s[gaussian_index] *1.0 / np.sqrt(np.prod(2 * np.pi * var_s[gaussian_index]))
        part2 = np.exp(-0.5 * np.sum((vector - mean_s[gaussian_index]) ** 2 * 1.0 / var_s[gaussian_index]))
        probability_total += weight_s[gaussian_index]*part1 * part2
    if probability_total == 0:
        return sys.float_info.max
    return - np.log(probability_total)
...
def back_pointer_dtw(mfcc_sequences, trellis):
    ...
    for node in trellis[1:]:    # the first node is "non_0"
        if node.father_ref[0].description == "non_0":
            node.cost = distance_gmm(mfcc_sequences[0], trellis[1].mean_s, trellis[1].var_s, trellis[1].weight_s)
        else:
            node.cost = sys.float_info.max
    ...
    for mfcc in mfcc_sequences[1:]:
        ...
        for node in trellis[1:]:
            if node.is_non_emmit:   # non-emmit state does not respond to any data
                ...
                observation_cost = distance_gmm(mfcc, node.mean_s, node.var_s, node.weight_s)
...
if __name__ == '__main__':
        ...
        test_samples = []
        for i in range(24):
            sample_file = "./junyo_short/junyo_short_" + str(i + 1) + ".txt"
            np_array = np.loadtxt(sample_file)
            test_samples.append(np_array.tolist())
        for i in range(26):
            sample_file = "./junyo_long/junyo_long_" + str(i + 1) + ".txt"
            np_array = np.loadtxt(sample_file)
            test_samples.append(np_array.tolist())
