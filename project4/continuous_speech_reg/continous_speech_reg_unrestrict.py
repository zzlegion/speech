...
import record
import readwave
import continous_speech_reg_help_util
LOOP_BACK_PENALTY = 0
PENALTY = 200
...
def back_pointer_dtw(mfcc_sequences, trellis):
    ...
    # current_mfcc_index = 0
    for mfcc in mfcc_sequences[1:]:
        # current_mfcc_index += 1
        ...
        # non_0 node is special, it has loop back penalty
        non_0_node = trellis[0]
        non_0_node.cost = non_0_node.father_ref[0].pre_cost + LOOP_BACK_PENALTY
        non_0_node.backPointer = non_0_node.father_ref[0].pre_backPointer
        # deal rest of the nodes for the new incoming mfcc
...
if __name__ == '__main__':
    ...
    for sample in test_samples:
        generated_trellis = trellis_generate("network_gen_unrestricted.txt")
...
# fake mfcc patch test
#     test_samples = ["911385", "2212", "123456", "55555", "37274921",
#                     "826414052002", "7343332190377", "6890372344", "72184347924"]
#     for sample in test_samples:
#         label, continuous_speech = continous_speech_reg_help_util.make_fake_mfcc(sample)
#         getresult = back_pointer_dtw(continuous_speech, generated_trellis)
#         # pending recognition result
#         pend = True
#         if len(getresult) != len(label):
#             pend = False
#         else:
#             for (digit_in_label, digit_in_result) in zip(label, getresult):
#                 if digit_in_label != digit_in_result:
#                     pend = False
#                     break
#         # print result
#         print(getresult, pend)
#         generated_trellis = trellis_generate("network_gen_unrestricted.txt")

# fake mfcc test
#     (label, continuous_speech) = continous_speech_reg_help_util.make_fake_mfcc("826414052002")
#     getresult = back_pointer_dtw(continuous_speech, generated_trellis)
#     # pending recognition result
#     pend = True
#     if len(getresult) != len(label):
