...
if __name__ == '__main__':
    ...
    sequence_num_digit = [0]*10   # 包含每个digit的sequence数量。sequence_num_digit[i]代表包含i的sequence数量

    f_label = open('../hwdata/smallTRAIN.transcripts', 'r')
    f_name = open('../hwdata/smallTRAIN.filelist', 'r')
    pattern = {'zero': 0, 'oh': 0, 'one': 1, 'two': 2, 'three': 3, 'four': 4, 'five': 5,
...
# if __name__ == '__main__':
#     train_data = []
#     train_data.append([1,2])  ## 俊优的用于训练的录音index
#     train_data.append([]) ## 健炜的用于训练的录音index
#     training_sequence_num = len(train_data[0]) * 6 + len(train_data[1])*6
#     sequence_num_digit = [0] * 10
#     for i in xrange(10):
#         sequence_num_digit[i] = training_sequence_num
#
#     align(train_data,'..\\speech model\\hmm continuous')
#
