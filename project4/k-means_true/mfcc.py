...
WINDOW_FUNCTION=[(0.54 - 0.46 * math.cos(2 * math.pi * i / (SAMPLE_SIZE - 1))) for i in xrange(SAMPLE_SIZE)]
...
def zero_padding(frame):
    for i in xrange(SAMPLE_SIZE, PADDING_SIZE):
        frame.append(0)
    return frame
...
def generate_mi():
    ...
    fftfrqs = []
    for i in xrange(PADDING_SIZE // 2 + 1):
        fre = i * SAMPLE_RATE / PADDING_SIZE
        fftfrqs.append(fre)
    minmel = hz2mel(MIN_FREQ)
    maxmel = hz2mel(MAX_FREQ)
    binfrqs = []
    for i in xrange(0, FILTER_NUM + 2):
        binfrqs.append(mel2hz(minmel + i * (maxmel - minmel) / (FILTER_NUM + 1)))
    MI = [[0 for i in xrange(PADDING_SIZE // 2 + 1)] for i in range(FILTER_NUM)]
    for i in xrange(FILTER_NUM):
        low = binfrqs[i]
        mid = binfrqs[i + 1]
        high = binfrqs[i + 2]
        for j in xrange(int(PADDING_SIZE / 2 + 1)):
...
def dct(log_mel_spectrum):
    mel_ceptrum = []
    for i in xrange(DCT_DIM):
        sum = 0
        for j in xrange(FILTER_NUM):
...
def mfcc(wave,mode="normal"):
    ...
    while sample_index <= sample_boundary:
        ...
        for i in xrange(FILTER_NUM):
            sum = 0
            for j in xrange(PADDING_SIZE // 2 + 1):
    ...
    for mel_ceptrum in mel_ceptrum_matrix:
        for i in xrange(mel_ceptrum_len):
            mean_mel_ceptrum[i]+=mel_ceptrum[i]
    mean_mel_ceptrum=map(lambda x: x/mel_ceptrum_matrix_len, mean_mel_ceptrum)
    # minus min
    for mel_ceptrum_index in xrange(mel_ceptrum_matrix_len):
        for mel_ceptrum_inner_index in xrange(mel_ceptrum_len):
    ...
    sd=map(lambda x: (x/mel_ceptrum_matrix_len)**0.5,sd)
    # devide sd
    for mel_ceptrum_index in xrange(mel_ceptrum_matrix_len):
        for mel_ceptrum_inner_index in xrange(mel_ceptrum_len):
    ...
    # extend ceptrum
    for i in xrange(mel_ceptrum_matrix_len-1):
